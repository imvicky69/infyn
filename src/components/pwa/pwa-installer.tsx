...
export function PWAInstaller() {
  ...
  useEffect(() => {
    ...
    // If already installed or running standalone, do not show install banners
    if (standaloneCheck) return;

    // Check if user dismissed banner within the last 5 days
    const dismissedTime = localStorage.getItem("infyn_pwa_banner_dismissed");
    if (dismissedTime) {
      const daysSinceDismissed =
        (Date.now() - parseInt(dismissedTime, 10)) / (1000 * 60 * 60 * 24);
      if (daysSinceDismissed < 5) return;
    }
  ...
  const handleDismissBanner = () => {
    setShowBanner(false);
    localStorage.setItem("infyn_pwa_banner_dismissed", Date.now().toString());
  };
        ...
        {showBanner && (
          ...
          >
            <div className="flex items-center gap-3.5 p-3.5 sm:p-4 rounded-2xl bg-white/95 dark:bg-[#141417]/95 backdrop-blur-xl border border-[#EAEAE5] dark:border-zinc-800 shadow-[0_12px_40px_rgba(0,0,0,0.12),0_2px_10px_rgba(0,0,0,0.06)]">
              {/* App Icon */}
              <div className="relative h-12 w-12 rounded-xl overflow-hidden bg-white border border-[#EAEAE5] dark:border-zinc-700 shrink-0 shadow-2xs flex items-center justify-center p-1.5">
                <Image
                  src="/icon-192.png"
                  alt="Infyn Logo"
                  width={40}
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-[9px] font-extrabold uppercase px-1.5 py-0.5 rounded-full bg-emerald-50 dark:bg-emerald-950/50 text-emerald-700 dark:text-emerald-400 border border-emerald-200/80 dark:border-emerald-800/60">
                    Free
                  </span>
                </div>
            ...
            >
              ...
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  <div className="h-12 w-12 rounded-2xl bg-white border border-[#EAEAE5] dark:border-zinc-700 p-1.5 flex items-center justify-center shadow-2xs">
                    <Image
                      src="/icon-192.png"
                      alt="Infyn"
                      width={40}
                  ...
                  <div>
                    <h3 className="text-base font-extrabold text-[#111111] dark:text-white flex items-center gap-1.5">
                      <span>Install Infyn Web App</span>
                    </h3>
                    <p className="text-xs text-[#6E6D68] dark:text-zinc-400 mt-0.5">
                      Run directly from your phone home screen
                    </p>
                  </div>
                </div>
