...
export default function PdfProtectorPage() {
  ...
  const [items, setItems] = useState<ProtectedFileResult[]>([]);
    ...
    for (const file of selected) {
      try {
        ...
        if (err.message?.toLowerCase().includes("encrypted") || err.message?.toLowerCase().includes("password")) {
          setErrorMsg(`"${file.name}" is already password-protected. Please unlock it using the PDF Unlocker tool first.`);
          return;
        }
      }
    }
  ...
  const handleEncryptFiles = async () => {
    ...
    if (!password.trim()) {
      setErrorMsg("Please enter a password to protect the PDFs.");
      return;
    }
    ...
    try {
      for (let i = 0; i < total; i++) {
        const file = files[i];
        setProgressText(`Encrypting ${file.name}...`);
        ...
        try {
          ...
          console.error(`Failed to encrypt ${file.name}:`, err);
          results.push({
            id: `${file.name}-${Date.now()}`,
            originalName: file.name,
            originalSize: file.size,
      ...
      setProgressText("Encryption complete!");
      setItems(results);
    } catch (err: any) {
  ...
  const handleDownloadAllZip = async () => {
    ...
    if (successfulItems.length === 1) {
      ...
      a.href = URL.createObjectURL(item.encryptedBlob);
    ...
    successfulItems.forEach((item) => {
      zip.file(item.outputFileName, item.encryptedBlob);
    });
    ...
    const a = document.createElement("a");
    a.href = downloadUrl;
    a.download = `infyn-protected-pdfs-${successfulItems.length}.zip`;
    a.click();
      ...
      <main className="flex-1 w-full max-w-5xl mx-auto px-4 sm:px-6 flex flex-col py-8">
          ...
          >
            <div className="text-center space-y-3">
              ...
              <p className="text-sm text-[#6E6D68] flex items-center justify-center gap-2 flex-wrap">
                <span>Secure PDFs with a password</span>
        ...
        {files.length > 0 && !isProcessing && items.length === 0 && (
          ...
          >
            ...
            <div className="flex items-center justify-between border-b border-[#EAEAE5] pb-4">
              <div>
                <h2 className="text-xl font-bold text-[#111111]">Secure Your Documents</h2>
                <p className="text-sm text-[#6E6D68]">Set a password to restrict access</p>
              </div>
            ...
            <div className="flex items-center gap-3 pt-4 border-t border-[#EAEAE5]">
              ...
              >
                ...
                Encrypt & Protect PDFs
              </button>
            </div>
            
            <PrivacyBadges badges={["AES-256 Encryption", "Zero cloud uploads", "Local execution"]} className="pt-4" />
          </div>
        )}
        ...
        {isProcessing && (
          <div className="flex-1 flex flex-col items-center justify-center py-12">
            <div className="w-full max-w-md space-y-6 bg-white p-8 rounded-3xl border border-[#EAEAE5] shadow-xs text-center">
              ...
              <h2 className="text-xl font-bold text-[#111111]">Securing Documents</h2>
        ...
        {!isProcessing && items.length > 0 && (
          <div className="space-y-6" style={{ animation: "fade-in-up 0.3s ease-out" }}>
              ...
              <div className="space-y-1 text-center md:text-left">
                <div className="flex flex-wrap items-center justify-center md:justify-start gap-2">
                  <span className="text-sm font-bold text-white">
                    {items.filter(i => i.status === "done").length} {items.length === 1 ? "File" : "Files"} Secured
                  </span>
                ...
                <p className="text-xs text-white/70">
                  Password required to open these documents.
                </p>
              </div>
            ...
            <PrivacyBadges badges={["100% In-browser", "Files never leave your device", "No watermarks", "AES-256 standard"]} className="pt-2" />
          </div>
        )}
      </main>
      <Footer />
